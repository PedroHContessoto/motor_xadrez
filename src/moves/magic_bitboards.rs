...
mod tests {
    ...
    fn test_magic_initialization() {
        ...
        // Teste básico - torre no centro
        let attacks = get_rook_attacks_magic(28, 0); // e4, tabuleiro vazio
        assert_ne!(attacks, 0);

        // Teste básico - bispo no centro
        let attacks = get_bishop_attacks_magic(28, 0); // e4, tabuleiro vazio
        assert_ne!(attacks, 0);
    }

    #[test]
    fn test_attack_consistency() {
        init_magic_bitboards();

        // Compara com método lento para verificar correção
        for square in 0..64 {
            let occupancy = 0x12345678ABCDEF00; // Ocupação teste

            let magic_rook = get_rook_attacks_magic(square, occupancy);
            let magic_bishop = get_bishop_attacks_magic(square, occupancy);

            assert_ne!(magic_rook, 0);
            assert_ne!(magic_bishop, 0);
        }
    }
}
...
mod benchmarks {
    ...
    #[test]
    fn benchmark_magic_vs_traditional() {
        init_magic_bitboards();

        let iterations = 1_000_000;
        let occupancy = 0x12345678ABCDEF00;

        // Benchmark magic bitboards
        let start = Instant::now();
        for _ in 0..iterations {
            for square in 0..64 {
                let _ = get_rook_attacks_magic(square, occupancy);
                let _ = get_bishop_attacks_magic(square, occupancy);
            }
        }
        let magic_time = start.elapsed();

        println!("Magic bitboards: {:?} for {} iterations", magic_time, iterations * 64 * 2);
